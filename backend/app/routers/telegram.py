...
1. This router cannot use `app/dependencies/auth.get_current_user` on the
   ...
   account. Two linking methods exist:
     - `/login <email> <password>` — verifies credentials directly
       against the same bcrypt hash used by `POST /auth/login`
       (`_handle_login_command` below) and links immediately. Rate
       limited per chat; the triggering message is deleted from the
       chat afterward to avoid leaving a plaintext password in history.
     - `/connect <code>` / `/start <code>` — a short-lived code
       generated by an authenticated web session (`_handle_start_command`
       below); see the module docstring on `telegram_link_store.py` for
       the full code-based linking flow.
...
from app.services.user_store import DynamoDBError as UserStoreDynamoDBError, get_user_by_email_raw
...
def _handle_start_command(chat_id: int, text: str) -> str:
    ...
    if len(parts) < 2 or not parts[1].strip():
        return (
            "Welcome to OBLIGRA! Connect your account one of two ways:\n"
            "1. /login <email> <password> — log in directly with your OBLIGRA credentials.\n"
            "2. /connect <code> — generate a code from the OBLIGRA web app (Settings > Telegram) "
            "and send it here."
        )
...
def _handle_login_command(chat_id: int, text: str) -> str:
    """Handle a `/login <email> <password>` message: verify credentials and link the chat.

    This is a second, direct way to link a Telegram chat to an OBLIGRA
    account — an alternative to the `/connect CODE` flow above, not a
    replacement for it. It reuses the exact same email+password
    verification as `POST /auth/login` (`app/services/security.verify_password`
    against the bcrypt hash in `app/services/user_store`); no separate
    credential-checking path is introduced.

    Rate-limited per chat (`telegram_link_store.is_login_rate_limited`)
    since, unlike the web login form, this command is reachable by
    anyone who opens a chat with the bot — there is no email-enumeration
    step required first. Returns the plain-text reply to send back to
    the user; the caller is responsible for deleting the triggering
    message afterward so the password doesn't linger in chat history.
    """
    if is_login_rate_limited(str(chat_id)):
        return (
            "Too many login attempts. Please wait a few minutes before trying again, or use "
            "/connect CODE instead (generate a code from the OBLIGRA web app)."
        )

    parts = text.split(maxsplit=2)
    if len(parts) < 3:
        return (
            "To log in, send: /login your-email@example.com your-password\n\n"
            "For your security, I'll delete this message right after processing it. "
            "You can also use /connect CODE instead (generate a code from the OBLIGRA web app)."
        )

    email, password = parts[1].strip(), parts[2]

    record_login_attempt(str(chat_id))

    try:
        raw_user = get_user_by_email_raw(email)
    except UserStoreDynamoDBError as exc:
        logger.exception("Failed to look up user during Telegram /login")
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Failed to process login. Please try again.",
        ) from exc

    # Same-message response for "no such email" and "wrong password" as
    # the web /auth/login endpoint, for the same reason: this must not
    # let a caller enumerate which emails are registered.
    invalid_credentials_reply = "Incorrect email or password. Please try again with /login <email> <password>."

    if raw_user is None:
        return invalid_credentials_reply
    ...
    try:
        ...
        logger.exception("Failed to link Telegram chat %s to user %s via /login", chat_id, user_id)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Failed to link your account. Please try again.",
        ) from exc

    return "You're logged in. This chat is now linked to your OBLIGRA account — send any message to extract obligations."
...
async def telegram_webhook(
    ...
    if is_photo:
        ...
        if user_id is None:
            reply = "Please connect your OBLIGRA account first: /login <email> <password>, or /connect CODE."
        else:
        ...
        if text.startswith("/start") or text.startswith("/connect"):
            ...
            reply = await asyncio.to_thread(_handle_login_command, chat_id, text)
            # Best-effort: scrub the plaintext password from the chat
            # history now that it's been checked. Never blocks or fails
            # the webhook response — see delete_message's docstring.
            try:
                await asyncio.to_thread(delete_message, chat_id, message.message_id)
            except TelegramError as exc:
                logger.warning("Failed to delete /login message in chat %s: %s", chat_id, exc)
        else:
            try:
                user_id = get_user_id_for_chat(str(chat_id))
            except TelegramLinkDynamoDBError as exc:
